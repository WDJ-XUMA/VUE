<template>
    <div>
        ...
        <div class="outer-swiper">
            <div class="swiper">
                <my-swipe :imgs="imgsUrl"></my-swipe>
            </div>
        </div>
        <div class="product-desc">
            <ul>
                ...
                <li class="number-li">购买数量：<span @click="subTract">-</span><span>{{num}}</span><span @click="addNum">+</span></li>
                <li>
...
<script>
    ...
    export default{
        ...
        created(){
            ...
            this.$axios.all([
                this.$axios.get('goodsImgs.json'),
                this.$axios.get('goodsDetail.json')
            ])
            .then(this.$axios.spread((res1,res2)=>{
                // console.log(res1);
                // console.log(res2);
                this.imgsUrl = res1.data.message[gId];
                this.goodsInfo = res2.data.message[gId];
            }))
            .catch(err=>{
                console.log(err);
            });
